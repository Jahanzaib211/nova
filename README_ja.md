...
Nova は DeerFlow 2.0 のフルスタックリファクタリングです——**新規 698 ファイル・120,533 行の新規コード**、総差分は **1,148 ファイル、+145,947 / −11,844**。フォーク時点の上流 1,338 ファイルのうち 884 ファイル（66%）は未変更です。監査日 2026-09-08。すべての数値は再現可能で、除外した生成物とその理由を含む正確なコマンドは **[NOVA_VS_DEERFLOW.md](./NOVA_VS_DEERFLOW.md)** にあります。

主な追加機能はすべて Nova 用に構築：
...
`trading` ツールグループは、無料かつAPIキー不要のソースから実際の市場データをエージェントに提供します:

| ツール | 機能 |
|---|---|
| `get_ohlcv` | OHLCVローソク足 —— 株式/FX/先物は **yfinance**、暗号資産は **ccxt** の公開エンドポイント |
| `compute_indicators` | SMA、EMA、RSI（Wilder）、MACD、ATR、ADX/DMI、ボリンジャーバンド、±kσバンド付きセッションVWAP |
| `backtest_signals` | 実際のローソク足でエントリーシグナルを再生 → 勝率、R単位の期待値、プロフィットファクター、最大ドローダウン |

APIキーは不要です。インジケーターは純粋なPython実装のためデフォルトインストールで動作します。
ライブデータの取得にはオプションの依存関係が1つ必要です:

```bash
cd backend && uv sync --extra trading    # yfinance + ccxt をインストール
```

1本のバーがストップとターゲットの両方に触れた場合は**損失**として扱います —— OHLCデータでは
バー内の価格順序を復元できず、有利な順序を仮定すると実運用で再現できない優位性を生み出すためです。

金について: 現物XAU/USDの無料フィードは存在せず、`XAUUSD`、`XAUUSD=X`、`XAU=X` はいずれも
Yahooで空のデータを返します。`GC=F`（COMEX先物）か、ccxt経由の `PAXG/USDT`（現物連動）を使ってください
—— 誤ったシンボルを指定するとツールが正しい候補を提示します。

これと対になるのが内蔵の **`pine-script`** スキルです。TradingView Pine Script v5/v6 の作成向けに、
言語リファレンス、現実的なバックテスト既定値を備えたインジケーター/ストラテジーのテンプレート、
そして TradingView に貼り付ける前に存在しない組み込み識別子を検出するバリデーターを提供します:
