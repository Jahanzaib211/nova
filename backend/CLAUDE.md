...
| **Admin** (`/api/v1/admin`) | Ops-console god-mode surface, gated by `require_admin_user` (real admin session or the `X-Nova-Ops-Token` service token — see `ops_auth.py`). `GET /users` / `/users/stats` - registration roster + growth metrics; `GET /users/{id}` - full standing (profile, credit balance, `recent_runs_for_user`); `PATCH /users/{id}/plan` - manual plan override; `POST /users/{id}/reset-usage`, `POST /users/{id}/grant`, `PATCH /users/{id}/limit` - credit controls; `GET /users/{id}/conversations` - list a user's threads (metadata only); `GET /users/{id}/conversations/{thread_id}/messages` - full message content for one of that user's threads (verifies thread ownership first, 404s otherwise); `POST /users/{id}/conversations/{thread_id}/runs/{run_id}/cancel` - cancel another user's in-flight run (same thread-ownership check, 404 on mismatch or unknown run, 409 if not cancellable); `GET /users/{id}/channels` - list a user's IM channel connections (no raw credentials); `DELETE /users/{id}/channels/{connection_id}` - revoke one of a user's channel connections; `GET /activity` - cross-user recent runs; `GET /audit` - operator audit trail, optional `target_user_id` query param to scope to one user's rows; `GET/POST /credit-requests` - self-service credit request inbox. The two conversation endpoints are the only surface that exposes private message text, so every read is written to the audit trail (`view-conversations` / `view-conversation-messages`) alongside every mutating action. Both require a SQL-backed `database.backend` (sqlite/postgres) — 503 otherwise, since there is nothing durable to page through in memory mode. The ops console UI itself is a separate external app, not part of this repo; this router is only its backend API surface |
