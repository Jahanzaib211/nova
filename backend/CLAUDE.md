...
| **Workspace** (`/api/workspace/{thread_id}`) | Feature-flagged (`workspace.intelligence_enabled`, default off). `POST /index` - build/refresh workspace snapshot (cost rate-limit tier); `GET /snapshot` - snapshot summary; `POST /plan` - read-only execution planning (search/run); `GET /commands` - detected command registry; `GET /symbols?q=` - symbol prefix search; `POST /impact` - blast radius of changed files; `GET /metrics` - kernel metrics. All owner-checked; scanned root derived server-side; plan execution NOT exposed over HTTP |
