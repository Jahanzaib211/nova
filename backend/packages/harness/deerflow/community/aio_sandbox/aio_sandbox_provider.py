...
# Container ports published per sandbox for the in-container dev-server preview.
# The first is the default ("app") preview; extras allow multi-port apps.
DEFAULT_PREVIEW_PORTS = [4100, 4101, 4102]
...
class AioSandboxProvider(SandboxProvider):
    ...
    def _load_config(self) -> dict:
        ...
        preview_ports = getattr(sandbox_config, "preview_ports", None)

        environment = self._resolve_env_vars(sandbox_config.environment or {})
        # GitHub connector (Phase 6): inject the operator token from the host env
        # into every sandbox container, but ONLY when it is actually set — read
        # directly from os.environ (never a `$GITHUB_TOKEN` in config.yaml, which
        # would hard-fail AppConfig loading when unset). Lets `git` push with
        # `http.extraHeader` inside the sandbox; a config-provided value wins.
        github_token = os.environ.get("GITHUB_TOKEN")
        if github_token and "GITHUB_TOKEN" not in environment:
    ...
    def get_preview_endpoint(self, thread_id: str, container_port: int = 4100) -> tuple[str, int] | None:
        """Resolve the published host endpoint for a thread's preview port.

        Returns ``(host, host_port)`` reachable from the gateway for the given
        in-container preview port, or ``None`` if the thread has no sandbox or
        that port was not published. Used by the dev-server preview proxy to
        target a dev server running inside the per-thread container.
        """
        from urllib.parse import urlparse

        with self._lock:
            sandbox_id = self._thread_sandboxes.get(thread_id)
            info = self._sandbox_infos.get(sandbox_id) if sandbox_id else None
            if info is None and sandbox_id is not None:
                warm = self._warm_pool.get(sandbox_id)
                info = warm[0] if warm is not None else None
        if info is None:
            return None

        host_port = (info.preview_ports or {}).get(container_port)
        if not host_port:
            return None
