...
def _thread_id_for_observation(sandbox_id: str) -> str | None:
    """Resolve the thread_id whose sandbox.log an observation should target.

    Local per-thread sandboxes encode it directly (``local:{thread_id}``). AIO /
    container sandboxes use opaque hash ids, so the thread is reverse-looked-up
    from the active provider's thread→sandbox map. The legacy global ``local``
...
def _write_sandbox_observation(
    ...
    Emits one JSON line per tool invocation so the frontend can render
    rich structured activity cards instead of raw terminal text.
...
_DEV_SERVER_RE = re.compile(
    ...
    r"serve|http-server|python3?\s+-m\s+http\.server|"
    # Custom long-running servers started directly (common entry-point filenames).
    r"(node|nodemon|tsx|ts-node|bun|deno\s+run)\s+\S*\b(server|app|index|main|start)\.(c|m)?(j|t)s)\b"
)


def _looks_like_dev_server(command: str) -> bool:
    """True for long-running dev-server commands (not build/lint/test)."""
    if "build" in command or "lint" in command or " test" in command:
        return False
    return bool(_DEV_SERVER_RE.search(command))


def _extract_dev_command_and_cwd(command: str, workspace_root: str) -> tuple[str, str]:
    """Split `cd <path> && <dev cmd>` into (bare_command, cwd).

    Strips `cd` prefixes, trailing `&`, `nohup`, and output redirects so the dev
    server can be launched cleanly under the async registry.
    """
    cwd = workspace_root
    cmd = command.strip()

    # Pull the last `cd <path>` target as the working directory.
    cd_matches = re.findall(r"cd\s+([^\s&;|]+)", cmd)
    if cd_matches:
    ...
    # Keep only the segment after the last `&&` (the actual dev command).
    if "&&" in cmd:
        cmd = cmd.split("&&")[-1].strip()

    # Strip backgrounding / nohup / redirects.
    cmd = re.sub(r"\s*>\s*\S+", "", cmd)
    cmd = re.sub(r"\s*2>&1", "", cmd)
    cmd = cmd.replace("nohup ", "").strip()
    cmd = cmd.rstrip("&").strip()
    # Strip a leading inline env-assignment prefix (e.g. "PORT=4200 npm run dev").
    # The dev-server launcher sets the correct PORT/HOST itself (the published
    # container port — an agent-chosen port would be unreachable through the
    # proxy), and a leading "VAR=val" would otherwise be exec'd as the program
    # by setsid/nohup ("failed to execute PORT=4200: No such file or directory").
    cmd = re.sub(r"^(?:[A-Za-z_][A-Za-z0-9_]*=\S+\s+)+", "", cmd).strip()
    return cmd, cwd


async def _bash_tool_async(runtime: Runtime, description: str, command: str) -> str:
    # Transparently route long-running dev-server commands to the async dev-server
    # registry so they don't block, get a stable port, and become previewable in
    # the Browser tab — regardless of whether the agent used start_dev_server.
    try:
        if _looks_like_dev_server(command):
            ...
            if is_local_sandbox(runtime):
                ...
                if thread_id:
                    ensure_thread_directories_exist(runtime)
                    thread_data = get_thread_data(runtime)
                    workspace_root = thread_data.get("workspace_path") or ""
                    if workspace_root:
                        ...
                        return (
                            f"✓ Dev server starting on port {handle.port} (command: {bare_cmd}).\n"
                            f"The Browser tab in Agent's Computer will show the live app once it compiles "
                            f"(~10-30s). Preview URL: /api/sandbox/preview/{thread_id}/.\n"
                            f"Do NOT run this command again or curl it to verify — the panel handles that."
                        )
            else:
                # Container sandbox (AIO): start the dev server INSIDE the container,
                # reached via its published preview port.
                thread_data = get_thread_data(runtime)
                ...
                if sandbox_id and thread_id:
                    ...
                    if sandbox is not None:
                        ...
                        return (
                            f"✓ Dev server starting in the thread's container (command: {bare_cmd}).\n"
                            f"The Browser tab in Agent's Computer will show the live app once it compiles "
                            f"(~10-30s). Preview URL: /api/sandbox/preview/{thread_id}/.\n"
                            f"Do NOT run this command again or curl it to verify — the panel handles that."
                        )
    except Exception:
        logger.debug("dev-server auto-route failed; falling back to blocking bash", exc_info=True)
...
def read_file_tool(
    ...
    try:
        ...
        sandbox_id = (runtime.state.get("sandbox") or {}).get("sandbox_id", "") if hasattr(runtime, "state") else ""
        _write_sandbox_observation(sandbox_id, "read_file", requested_path, f"Read {len(content)} chars")
        return result
    except SandboxError as e:
        return f"Error: {e}"
    except FileNotFoundError:
