...
def test_tool_freq_resets_per_run():
    ...
    for i in range(5):
        mw._apply(_make_state(tool_calls=[_bash_call(f"cmd{i}")]), runtime)
    assert mw._tool_freq["test-thread"]["bash"] == 5

    # A new run (before_agent) resets the per-tool budget for this thread.
    mw.before_agent(_make_state(), runtime)
    assert mw._tool_freq.get("test-thread", {}).get("bash", 0) == 0

    # And the counter starts fresh on the next run.
    mw._apply(_make_state(tool_calls=[_bash_call("again")]), runtime)
    assert mw._tool_freq["test-thread"]["bash"] == 1
