...
@pytest.mark.anyio
async def test_create_or_reject_reaps_stale_dead_task_run():
    """A run whose task finished without a clean status transition must not wedge
    the thread behind a permanent 409 — create_or_reject should reap it."""
    manager = RunManager()
    stale = await manager.create_or_reject("thread-stale", multitask_strategy="reject")

    # Simulate a dead worker: status still 'running' but the asyncio task is done.
    async def _noop():
        return None
    ...
    # A new run on the same thread should succeed (stale reaped → interrupted).
    fresh = await manager.create_or_reject("thread-stale", multitask_strategy="reject")
    assert fresh.status == RunStatus.pending
    assert stale.status == RunStatus.interrupted


@pytest.mark.anyio
async def test_create_or_reject_still_rejects_genuinely_active_run():
    """A run with a live (not-done) task must still trigger the 409 reject."""
    manager = RunManager()
    active = await manager.create_or_reject("thread-active", multitask_strategy="reject")
