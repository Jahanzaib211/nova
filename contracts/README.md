...
| `capabilities.baseline.json` | The capability registry snapshot: every module and operation with its kind, flags and input/output JSON schemas. Source of the generated frontend client (`frontend/src/core/capabilities/generated.ts`) and of the MCP tool list; removed or retyped operations fail the build, additions pass | `backend/tests/test_capability_registry.py`, `frontend/tests/unit/core/capabilities/api.test.ts`, `backend/scripts/gen_capabilities_client.py --check` (lint-check.yml) |
