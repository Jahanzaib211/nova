...
async def create_sandbox(req: CreateSandboxRequest):
    ...
    try:
        ...
        logger.info(f"Created Pod {_pod_name(sandbox_id).replace(chr(10), '').replace(chr(13), '')}")
    except ApiException as exc:
    ...
    try:
        ...
        logger.info(f"Created Service {_svc_name(sandbox_id).replace(chr(10), '').replace(chr(13), '')}")
    except ApiException as exc:
        if exc.status != 409:
...
async def destroy_sandbox(sandbox_id: str):
    ...
    try:
        ...
        logger.info(f"Deleted Service {_svc_name(sandbox_id).replace(chr(10), '').replace(chr(13), '')}")
    except ApiException as exc:
        if exc.status != 404:
    ...
    try:
        core_v1.delete_namespaced_pod(_pod_name(sandbox_id), K8S_NAMESPACE)
        logger.info(f"Deleted Pod {_pod_name(sandbox_id).replace(chr(10), '').replace(chr(13), '')}")
    except ApiException as exc:
        if exc.status != 404:
