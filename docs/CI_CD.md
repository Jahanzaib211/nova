...
|---|---|---|
| `backend-unit-tests.yml` | push/PR to main | 6,496 backend unit tests (`make test`) |
| `backend-blocking-io-tests.yml` | push/PR to main | Blockbuster runtime gate on async blocking IO |
| `frontend-unit-tests.yml` | push/PR to main | 575 frontend unit tests (`pnpm test`) |
...
| Suite | Count | Command |
|---|---|---|
| Backend unit tests | 6,496 | `cd backend && make test` |
| Frontend unit tests | 575 | `cd frontend && pnpm test` |
