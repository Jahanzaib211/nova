...
Nova's CI/CD runs on GitHub Actions with 15 workflows. You can also run the full CI locally using `nektos/act`.
...
|---|---|---|
| `backend-unit-tests.yml` | push/PR to main | 6,430 backend unit tests (`make test`) |
| `backend-blocking-io-tests.yml` | push/PR to main | Blockbuster runtime gate on async blocking IO |
| `frontend-unit-tests.yml` | push/PR to main | 565 frontend unit tests (`pnpm test`) |
