...
Nova's CI/CD runs on GitHub Actions with 16 workflows. You can also run the full CI locally using `nektos/act`.
...
|---|---|---|
| `backend-unit-tests.yml` | push/PR to main | 6,489 backend unit tests (`make test`) |
| `backend-blocking-io-tests.yml` | push/PR to main | Blockbuster runtime gate on async blocking IO |
| `frontend-unit-tests.yml` | push/PR to main | 568 frontend unit tests (`pnpm test`) |
...
| Suite | Count | Command |
|---|---|---|
| Backend unit tests | 6,489 | `cd backend && make test` |
| Frontend unit tests | 568 | `cd frontend && pnpm test` |
