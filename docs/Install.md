...
This file is for coding agents. If the Nova repository is not already cloned and open, clone `https://github.com/Jahanzaib211/nova.git` first, then continue from the repository root.

## Goal

Bootstrap a Nova local development workspace on the user's machine with the least risky path available.
...
- The Nova repository is cloned and the current working directory is the repo root.
- `config.yaml` exists.
...
- The user receives the exact next command to launch Nova.
...
- If the current directory is not the Nova repository root, clone `https://github.com/Jahanzaib211/nova.git` if needed, then change into the repository root.
- Confirm the current directory is the Nova repository root by checking that `Makefile`, `backend/`, `frontend/`, and `config.example.yaml` exist.
...
5. Exact next command to start Nova
