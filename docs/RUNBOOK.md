...
Users report `Error 1033 — Cloudflare cannot resolve the tunnel`. The
gateway is up; the public URL returns 502 from nginx because the upstream
is unreachable from the tunnel's perspective.
...
If the watchdog reports `sudoers is missing 'reset-failed
cloudflared-nova'` in its logs, the operator must apply the sudoers update:
