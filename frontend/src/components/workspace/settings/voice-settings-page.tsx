...
import { AlertTriangleIcon, CheckCircle2Icon, Loader2Icon, MicIcon, RotateCcwIcon, Volume2Icon } from "lucide-react";
...
const DEVICES = [
  { value: "auto", label: "Auto", hint: "Use the GPU when it is really available, CPU otherwise." },
  { value: "cuda", label: "GPU (CUDA)", hint: "Falls back to CPU with a warning if CUDA is unusable." },
...
function Choice({ options, value, onChange }: { options: { value: string; label: string }[]; value: string; onChange: (v: string) => void }) {
  return (
    <div className="flex gap-1">
      {options.map((o) => (
        <Button key={o.value} size="sm" variant={value === o.value ? "default" : "outline"} onClick={() => onChange(o.value)} className="h-7 text-xs">
          {o.label}
        </Button>
      ))}
    </div>
...
export function VoiceSettingsPage() {
  ...
  const refresh = useCallback(async (probe = false) => {
    try {
      const [cfg, st] = await Promise.all([getVoiceConfig(), getVoiceStatus(probe)]);
      setConfig(cfg);
      setStatus(st);
      setLoadError(null);
    } catch (e) {
  ...
  if (loadError) {
    return (
      <SettingsSection title="Voice" description="Talk to Nova, and Nova talks back.">
      ...
      <div className="space-y-6">
        ...
        <div
          className={cn(
            ...
            enabled && ready ? "border-emerald-500/30 bg-emerald-500/5" : "border-amber-500/30 bg-amber-500/5",
          )}
        >
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">
              {!enabled ? "Voice is off" : ready ? "Voice is ready" : "Voice is on, but the engines are not loading"}
            </span>
          </div>
          {status?.reason && <p className="text-muted-foreground mt-1.5 font-mono text-xs">{status.reason}</p>}
          {config.live && (
            <div className="text-muted-foreground mt-2 grid gap-1 text-xs sm:grid-cols-2">
              {(["stt", "tts"] as const).map((k) => {
                const live = config.live![k];
                const fellBack = live.device_requested === "cuda" && live.device_actual !== "cuda";
                return (
                  <div key={k} className="flex items-center gap-1.5">
                    <span className="uppercase opacity-60">{k}</span>
                    <span className="font-medium">{live.name}</span>
                    {live.device_actual && (
                      <Badge variant={live.device_actual === "cuda" ? "default" : "secondary"} className="h-4 px-1.5 text-[10px]">
                        {live.device_actual}
                      </Badge>
                    )}
                    {fellBack && <span className="text-amber-500">asked for GPU, running on CPU</span>}
                  </div>
                );
              })}
            </div>
        ...
        <div className="divide-border/50 divide-y">
          <Row label="Enable voice" hint="Off by default. Turning this on loads the models, which takes a few seconds.">
        ...
        <div>
          ...
          {sttChoice?.languages && <p className="text-muted-foreground mb-2 text-xs">{sttChoice.languages}</p>}
          <div className="divide-border/50 divide-y">
            <Row label="Model" hint="Bigger is more accurate and slower. On a GPU, accuracy becomes affordable.">
              <Choice
                options={(sttChoice?.models ?? []).map((m) => ({ value: m, label: m }))}
                value={stt.model ?? "base"}
                onChange={(v) => section("stt", { model: v })}
              />
            </Row>
            <Row label="Device" hint={DEVICES.find((d) => d.value === (stt.device ?? "auto"))?.hint}>
              <Choice options={DEVICES} value={stt.device ?? "auto"} onChange={(v) => section("stt", { device: v })} />
            </Row>
            <Row label="Language" hint="Blank auto-detects. Setting it explicitly is faster and more accurate.">
              <Input
                ...
                onBlur={(e) => {
                  ...
                  if (v !== (stt.language ?? "")) void section("stt", { language: v || null });
                }}
              />
            </Row>
          </div>
        ...
        <div>
          ...
          {ttsChoice?.note && <p className="text-muted-foreground mb-2 text-xs">{ttsChoice.note}</p>}
          <div className="divide-border/50 divide-y">
            <Row label="Engine">
              <Choice
                options={config.catalog.tts.map((c) => ({ value: c.use, label: c.label }))}
            ...
            <Row label="Voice">
              <Choice
                options={(ttsChoice?.voices ?? []).slice(0, 4).map((v) => ({ value: v, label: v }))}
        ...
        <div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              <Button size="sm" variant="outline" onClick={runSpeakerTest} disabled={!enabled || busy !== null}>
                {busy === "speaker" ? <Loader2Icon className="size-3.5 animate-spin" /> : <Volume2Icon className="size-3.5" />}
                Test speaker
              </Button>
              {speaker && (
                  ...
                  {speaker.ok ? (
                    <>
                      ...
                      {speaker.rtf != null && (
                        <>
                          {" · "}
                          <span className={speaker.rtf > 1 ? "text-amber-500" : undefined}>
            ...
            <div className="flex items-center gap-3">
              <Button size="sm" variant="outline" onClick={runMicTest} disabled={!enabled || busy !== null}>
                {busy === "mic" ? <Loader2Icon className="size-3.5 animate-spin" /> : <MicIcon className="size-3.5" />}
                Test microphone
              </Button>
              {mic && (
                <span className={cn("text-xs", mic.ok ? "text-muted-foreground" : "text-destructive")}>
                  {mic.ok ? (mic.heard ? `heard: “${mic.heard}”` : "recorded, but no words were recognised") : mic.error}
                </span>
              )}
            </div>
          </div>
        ...
        {config.has_overrides && (
          <div className="border-border/50 flex items-center justify-between gap-4 border-t pt-4">
            <p className="text-muted-foreground text-xs">
              These settings are saved separately from <code className="font-mono">config.yaml</code>, which is never modified.
            </p>
            <Button
              size="sm"
              variant="ghost"
