...
| `pine-script`                  | TradingView Pine Script v5/v6 authoring, with a validator that catches invented built-ins |
| `bootstrap`                    | Guided onboarding that generates a personalized `SOUL.md` for a custom agent     |
| `skill-creator`                | Create, modify, and evaluate skills                                              |
| `find-skills`                  | Discover and install skills matching what the user is trying to do               |
| `code-reviewer`                | Review changes for bugs, security issues, and quality                            |
| `qa-tester`                    | Test the current project and report what fails                                   |
| `file-organizer`               | Organize, sort, and clean up a folder                                            |
| `music-generation`             | Music and song generation workflows                                              |
| `vercel-deploy-claimable`      | Deploy applications and sites to Vercel                                          |
| `claude-to-deerflow`           | Drive a running Nova instance from Claude Code over its HTTP API                 |
| `surprise-me`                  | Combine available capabilities into an unexpected demo                           |
