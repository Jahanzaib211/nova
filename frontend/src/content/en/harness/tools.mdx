...
The `trading` group gives the agent real market data from free, keyless sources.
...
| `backtest_signals`    | Replays entry signals against real candles → win rate, expectancy in R, profit factor      |
...
The indicators are pure Python, so `compute_indicators` and `backtest_signals` work on a
default install. Fetching live data needs one optional extra:

```bash
cd backend && uv sync --extra trading   # yfinance + ccxt
```

Without it, `get_ohlcv` returns an install hint rather than an import traceback.

A bar that touches both stop and target is scored as a **loss** — OHLC data cannot reveal
intrabar order, and assuming the favourable one manufactures edge that will not survive live
trading. The count of such bars is reported so you can judge the impact.

There is no free spot XAU/USD feed: `XAUUSD`, `XAUUSD=X`, and `XAU=X` all return an empty
frame from Yahoo. Use `GC=F` / `MGC=F` (COMEX futures, ~1% above spot from carry), or
`PAXG/USDT` / `XAUT/USDT` via `source: ccxt` for a spot-tracking series. Yahoo FX pairs need
the `=X` suffix (`EURUSD=X`, not `EURUSD`).

See also the bundled [`pine-script` skill](/harness/skills) for turning a validated strategy
into TradingView Pine Script.
