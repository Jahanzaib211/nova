...
| `pine-script`                  | TradingView Pine Script v5/v6 编写，附带可捕获虚构内置标识符的校验器 |
| `bootstrap`                    | 引导式入门对话，为自定义智能体生成个性化的 `SOUL.md` |
| `skill-creator`                | 创建、修改和评估技能                         |
| `find-skills`                  | 根据用户意图发现并安装技能                   |
| `code-reviewer`                | 审查代码变更中的缺陷、安全问题和质量         |
| `qa-tester`                    | 测试当前项目并报告失败项                     |
| `file-organizer`               | 整理、分类和清理文件夹                       |
| `music-generation`             | 音乐和歌曲生成工作流                         |
| `vercel-deploy-claimable`      | 将应用和网站部署到 Vercel                    |
| `claude-to-deerflow`           | 通过 HTTP API 从 Claude Code 驱动运行中的 Nova 实例 |
| `surprise-me`                  | 组合现有能力生成出人意料的演示               |
