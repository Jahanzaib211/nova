...
function splitInlineReasoning(content: string) {
  ...
  // Orphan closing tag: some providers (e.g. MiniMax M3) emit a lone `</think>`
  // when the reasoning body is delivered via a separate `reasoning_content`
  // field — leaving a stray closer at the start of `content`. All real pairs
  // were removed above, so any remaining `</think>` is orphaned; strip it
  // (skip backtick-preceded mentions inside inline code).
  cleaned = cleaned.replace(/(^|[^`])<\/think>/g, "$1").trim();
