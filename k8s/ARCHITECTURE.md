...
### Confirmed real gap: K8s secrets vs. Compose secrets

`k8s/scripts/bootstrap-secrets.sh` populates exactly 10 keys in `nova-secrets`:
```
...
Compose's `env_file: ../.env` pattern (`docker-compose.yaml:154-155,192-193`) implicitly grants the container access to **every** variable in root `.env`, including ones never bootstrapped into K8s: `DATABASE_URL`, `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET`/`STRIPE_PRICE_ID_PLUS`, `NOVA_BYOK_SECRET`, `AUTH_JWT_SECRET`, `GITHUB_TOKEN`, `LANGSMITH_*`, most LLM/search provider keys (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, etc.), and **every IM channel credential** (`SLACK_BOT_TOKEN`, `TELEGRAM_BOT_TOKEN`, `DISCORD_BOT_TOKEN`, `FEISHU_APP_ID`/`SECRET`, `WECOM_*`, `DINGTALK_*`). **Consequence**: Stripe billing, BYOK, and every IM channel silently don't work in K8s staging today. Fixed in Phase 6.
