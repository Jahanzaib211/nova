...
# --- Auth: login and get session + CSRF cookies if credentials are set ---
_COOKIE_JAR=$(mktemp)
...
_deerflow_login() {
  ...
  if [ "$http_code" -ge 200 ] && [ "$http_code" -lt 300 ]; then
    ...
    echo "WARNING: DeerFlow login failed (HTTP ${http_code}) — trying unauthenticated" >&2
  fi
}
...
if [ -z "$THREAD_ID" ]; then
  THREAD_RESP=$(_curl -s -X POST "${LANGGRAPH_URL}/threads" \
...
TMPFILE=$(mktemp)
trap "rm -f '$_COOKIE_JAR' '$TMPFILE'" EXIT

_curl -s -N -X POST "${LANGGRAPH_URL}/threads/${THREAD_ID}/runs/stream" \
  -H "Content-Type: application/json" \
  -d "$BODY" > "$TMPFILE"
